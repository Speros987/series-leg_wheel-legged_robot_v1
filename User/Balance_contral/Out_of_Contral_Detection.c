...
{
    left_leg_off_ground_detection_state_vector.x_shen = -gyro[1];
    left_leg_off_ground_detection_state_vector.y_shen = gyro[2];


    //预留轮子加速度检测函数
    float F1 = 0, F2 = 0;
    F2 = left_u.tao_shen/(L_luntui + L_shentui) ;
    ...
    if((left_leg_off_ground_detection_state_vector.dd_x_lun -  (-N/m_lun) < (-N/m_lun)*0.5f) && (left_leg_off_ground_detection_state_vector.dd_x_lun - (-N/m_lun) > -((-N)/m_lun)*0.5f))
    {
        x_flag = 1 ;
    }
    else
    ...
    if(((left_leg_off_ground_detection_state_vector.dd_y_lun - ((P + m_lun*g)/m_lun) < ((P + m_lun*g)/m_lun)*0.5f) && (left_leg_off_ground_detection_state_vector.dd_y_lun - ((P + m_lun*g)/m_lun) > -((P + m_lun*g)/m_lun)*0.5f)) || left_leg.L_tui >= L_leg_max || left_leg.L_tui <= L_leg_min)
    {
        y_flag = 1 ;
    }
    else
    ...
    pre_velocity = CAN_1.velocity[2] ;
    float d_velocity = (CAN_1.velocity[2] - pre_velocity)/DT ;
    if (((d_velocity - (CAN_1.torque[2]/I_lun)) < (CAN_1.torque[2]/I_lun)*0.1f) || ((d_velocity - (CAN_1.torque[2]/I_lun)) > -(CAN_1.torque[2]/I_lun)*0.1f))
    {
        I_flag = 1 ;
    }
...
void right_wheel_acceleration_detection ()
{
    ...
    if((right_leg_off_ground_detection_state_vector.dd_x_lun -  (-N/m_lun) < (-N/m_lun)*0.1f) || (right_leg_off_ground_detection_state_vector.dd_x_lun - (-N/m_lun) > -((-N)/m_lun)*0.1f))
    {
        x_flag = 1 ;
    }
    else
    ...
    if((right_leg_off_ground_detection_state_vector.dd_y_lun - ((P + m_lun*g)/m_lun) < ((P + m_lun*g)/m_lun)*0.1f) || (right_leg_off_ground_detection_state_vector.dd_y_lun - ((P + m_lun*g)/m_lun) > -((P + m_lun*g)/m_lun)*0.1f))
    {
        y_flag = 1 ;
    }
    else
    ...
    if (((d_velocity - (CAN_2.torque[3]/I_lun)) < (CAN_2.torque[3]/I_lun)*0.1f) || ((d_velocity - (CAN_2.torque[3]/I_lun)) > -(CAN_2.torque[3]/I_lun)*0.1f))
    {
        I_flag = 1 ;
    }
